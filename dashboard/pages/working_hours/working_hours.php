...
<head>
    ...
    <title>Working Hours Management</title>
...
<body>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">Update Working Hours</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class='modal-body' id="update_working_hours">
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="add" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="staticBackdropLabel">Add Working Hours</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class='modal-body'>
                    ...
                    <div class='mb-3'>
                        ...
                        <input type='text' class='form-control' id='start_time' placeholder='Enter Menu Name'>
                    </div>
                    <div class='mb-3'>
                        <label for='exampleFormControlInput1' class='form-label'>End Time</label>
                        <input type='text' class='form-control' id='end_time' placeholder='Enter Menu Name'>
                    </div>
                    <div align='right'>
                        <button type='button' class='btn btn-primary' data-bs-dismiss='modal' onclick="add_working_hours()">Add</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function loadtable() {
            var url = 'working_hours_ajax.php'
            $.ajax({
                type: "POST",
                url: url,
                dataType: 'json',
                delay: 250,
                data: {
                    source: 'get_workinghours',
                },
                success: function(result) {
                    $("#working_table").html(result);
                },
            });
        }
        ...
        function working_details(id) {
            ...
            $.ajax({
                ...
                delay: 250,
                data: {
                    source: 'workingdetails',
                    id: id,
                },
                success: function(result) {
                    $('#update_working_hours').html(result);
                },
            });
        }

        function update_working_hours(id) {
            var url = 'working_hours_ajax.php'
            let day = $('#day_' + id).val();
